...
class Util:
    ...
    def transform_data_for_chart(attribute_data_out):
        ...
        if attribute_data_out.get("error"):
            return {"type": "unknown", "error": attribute_data_out["error"]}

        try:
            decoded_data = Util.decode_response(attribute_data_out["data"])

            if not decoded_data:
                return {"type": "unknown", "error": "Could not decode response"}
            ...
            if detected_type == KindMinorEnum.TABULAR.value:
                ...
                return {"type": "unknown", "data": data_dictionary}

        except Exception as e:
            return {"type": "unknown", "error": f"Failed to transform data: {str(e)}"}
